...
export default function MainUpload() {
	...
	const handleRestoreAutosave = async () => {
		...
		try{
			...
			setAutosaveError((error as Error).message || "Unable to restore autosaved session.")
	...
	const handleDiscardAutosave = async () => {
		try{
			...
			setAutosaveError((error as Error).message || "Unable to delete autosaved session.")
			setAutosaveStatus("error")
		}
	}
	...
	const skip_frame_forward = (_e?) =>{
		goToFrame(currframe_redux + skip_value)
	}

	const skip_frame_backward = (_e?) => {
		goToFrame(currframe_redux - skip_value)
	}
	...
	useEffect(() => {
		...
		autosaveTimeoutRef.current = setTimeout(async () => {
			...
			try{
				...
				setAutosaveError((error as Error).message || "Unable to save this session locally.")
				setAutosaveStatus("error")
			}
